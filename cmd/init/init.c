...
/* packages, and point cmake at @c{<venv>/bin/python3} via             */
...
/** @c{<ice_home>/venv/bin/python3} -- value passed as Python3_EXECUTABLE. */
static void venv_python(struct sbuf *out)
{
	sbuf_addf(out, "%s/venv/bin/python3", ice_home());
}
...
{
	const char *argv[] = {
	    py, "-c", "import site; print(site.getsitepackages()[0])", NULL,
	};
	struct process proc = PROCESS_INIT;
	char buf[4096];
	size_t len = 0;
...
static void write_sitecustomize(const char *site_packages, const char *ice_path)
{
	...
	sbuf_addf(&content,
		  ...
		  "    if base == \"ldgen.py\":\n"
		  "        os.execv(_ICE, [_ICE, \"idf\", \"ldgen\", *sys.argv[1:]])\n"
