...
static int cmd_tools_install(int argc, const char **argv);
...
static const struct cmd_manual tools_install_manual = {
	...
	.examples =
	H_EXAMPLE("ice tools install tools/tools.json")
	H_EXAMPLE("ice tools install --tool xtensa-esp-elf tools/tools.json")
	H_EXAMPLE("ice tools install --force tools/tools.json"),
};
/* clang-format on */
...
static const struct option cmd_tools_install_opts[] = {
...
static const struct cmd_desc cmd_tools_install_desc = {
    .name = "install",
    .fn = cmd_tools_install,
    .opts = cmd_tools_install_opts,
...
/**
 * Base directory for installation: either --path or @c ice_home().
 * Named install_base_dir so it does not collide with the sbuf variable
 * named @c tools_dir in cmd_tools_list below.
 */
static const char *install_base_dir(void)
{
...
static int install_from_manifest(const char *manifest_path,
