...
/*
 * Tiny glob matcher -- handles `*`, `?`, `[seq]`, `[!seq]`.  POSIX
 * fnmatch() isn't portable to MSVC / non-MinGW Windows toolchains,
 * and we only need the four basic wildcards (matching upstream's
 * --filter PATTERN documentation), so a 30-line recursive matcher
 * is simpler than dragging in a platform shim.
 */
...
static int glob_match(const char *pat, const char *str)
{
	while (*pat) {
		...
		if (*pat == '[') {
			...
			if (!*str)
				return 0;
			if (!char_in_class(pat + 1, end, *str))
				return 0;
			pat = end + 1;
			str++;
			continue;
